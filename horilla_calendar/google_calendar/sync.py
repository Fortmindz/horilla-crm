...
def _strip_legacy_crm_task_prefix(title):
...
def activity_to_google_event(activity):
    """
    Convert a horilla_activity.Activity to a Google Calendar Events resource.
    ...
    return {
        ...
        "extendedProperties": {
            "private": {
                "horilla_activity_id": str(activity.pk),
...
def push_activity_to_google(activity, user):
    ...
    try:
        if activity.activity_type == "task":
            ...
            # Mark CRM-pushed tasks so pull can sync completion only (no title overwrite),
            ...
            if not isinstance(ai, dict):
                ...
                ai = {**ai}
            ai["google_task_pushed_from_crm"] = True
...
def pull_google_events_to_horilla(config, initial_sync_only=False):
    ...
    Calendar event mapping:
      ...
      Horilla-originated events (have horilla_activity_id in extendedProperties) → skipped
    ...
    Google Tasks mapping:
      ...
      CRM-pushed tasks (additional_info.google_task_pushed_from_crm, or legacy "[Horilla] …" title)
    ...
    for gevent in events:
        ...
        if extended.get("horilla_activity_id") or extended.get(
            "horilla_unavailability_id"
        ):
    ...
    for gtask in gtasks:
        ...
        crm_pushed = (
            isinstance(ai, dict) and ai.get("google_task_pushed_from_crm") is True
        )
        # Legacy: titles used to be prefixed with "[Horilla] " to mark CRM-originated tasks.
        legacy_prefix = raw_title.startswith("[Horilla] ")

        # Tasks pushed from this CRM: only sync completion (avoid overwriting subject/body).
        if existing_task_activity and (crm_pushed or legacy_prefix):
            ...
            try:
                ...
                if not updated:
                    logger.debug(
                        "Google task %s marked CRM-owned but no matching Activity row",
                        google_task_id,
                    )
            except Exception as exc:
                logger.error(
        ...
        subject = _strip_legacy_crm_task_prefix(raw_title)[:100]
